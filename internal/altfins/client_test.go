...
func TestSignalsSearchBuildsRequest(t *testing.T) {
	...
	if page.Content[0].Bullish == nil || !*page.Content[0].Bullish {
		t.Fatalf("expected bullish response field to decode, got %#v", page.Content[0].Bullish)
	}
	if got, want := page.Content[0].DirectionLabel(), "BULLISH"; got != want {
		t.Fatalf("direction label mismatch: got %q want %q", got, want)
	}
}
